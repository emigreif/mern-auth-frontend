...
export default function ModalImportarAccesoriosOV({ obra, onClose, onCreated }) {
  ...
  const agregarManual = () => {
    ...
    if (!codigo || !descripcion || cantidad <= 0) return alert("Completa los campos obligatorios");
      ...
      <div className={styles.container}>
        ...
        <section className={styles.section}>
          ...
          <div className={styles.form}>
            <input type="text" placeholder="Código" value={nuevo.codigo} onChange={(e) => setNuevo({ ...nuevo, codigo: e.target.value })} />
            <input type="text" placeholder="Descripción" value={nuevo.descripcion} onChange={(e) => setNuevo({ ...nuevo, descripcion: e.target.value })} />
            <input type="text" placeholder="Color" value={nuevo.color} onChange={(e) => setNuevo({ ...nuevo, color: e.target.value })} />
            <input type="number" placeholder="Cantidad" value={nuevo.cantidad} onChange={(e) => setNuevo({ ...nuevo, cantidad: e.target.value })} />
            <input type="text" placeholder="Unidad" value={nuevo.unidad} onChange={(e) => setNuevo({ ...nuevo, unidad: e.target.value })} />
